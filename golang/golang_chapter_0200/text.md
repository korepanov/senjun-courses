...
Прежде чем рассматривать сборщик мусора, поговорим о памяти, которую сборщик мусора **не** очищает. Сборщик мусора не очищает значения локальных переменных. Например, значения переменных типа `int` или `float64` внутри функции. Такие переменные хранятся на **стеке** горутины. Компилятор Go сам может определить, когда их необходимо очистить. Для этого он генерирует соответствующие машинные инструкции. Обратите внмание, что это не касается указателей.
